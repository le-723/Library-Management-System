...
public class LendController {
    ...
    public ModelAndView lendList(HttpServletRequest request) {
        ...
        ArrayList<Lend> arrayLend= lendService.lendList();
        ArrayList<String> arrayBookname = new ArrayList<>();
        for (int i=0;i<arrayLend.size();i++){
            Book book =bookService.getBook(arrayLend.get(i).getBookId());
            arrayBookname.add(i,book.getName());
        }
        modelAndView.addObject("list",arrayLend);
        modelAndView.addObject("bookname",arrayBookname);
        return modelAndView;
    }
    ...
    public ModelAndView myLend(HttpServletRequest request) {
        ...
        ArrayList<Lend> arrayLend = lendService.myLendList(readerCard.getReaderId());
        ArrayList<String> arrayBookname = new ArrayList<>();
        for (int i=0;i<arrayLend.size();i++){
            Book book =bookService.getBook(arrayLend.get(i).getBookId());
            arrayBookname.add(i,book.getName());
